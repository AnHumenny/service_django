...
from .models import Users, SearchKey, SearchAccident
from info.models import Info
from datetime import date
from .serializers import UsersSerializer, InfoSerializer, InfoSerializerPrevios, KeySerializer, AccidentSerializer
...
class ViewOpenAccident(viewsets.ModelViewSet):
    queryset = SearchAccident.objects.all().filter(status="open").order_by("-id")
    serializer_class = AccidentSerializer

class ViewCheckAccident(viewsets.ModelViewSet):
    queryset = SearchAccident.objects.all().filter(status="check").order_by("-id")
    def create(self, request, *args, **kwargs):
        return Response({"detail": "Действие запрещено!"}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
